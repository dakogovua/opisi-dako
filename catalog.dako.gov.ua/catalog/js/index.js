var HIMAJIN = HIMAJIN || { InstancePool: {}, ShaderPool: {}, };

//------------------------------------------------------------

HIMAJIN.ParticleTexture = "data:image/png;base64,";

//------------------------------------------------------------
// HIMAJIN.Helper class
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool._ScreenQuadVS variable
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.Particle shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.DepthPass1 shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.DepthPass2 shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.DepthPass3 shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.ColorPass1 shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.ShaderPool.ColorPass2 shader
//------------------------------------------------------------
...
//------------------------------------------------------------
// HIMAJIN.AnimationFrameInfo class
//------------------------------------------------------------
...
HIMAJIN.Demo = (function () {
  ...
  _Class.prototype = {
    ...
    _InitPostProcessor: function () {
      ...
      this._DepthPass1Uniforms = THREE.UniformsUtils.clone(HIMAJIN.ShaderPool.DepthPass1.Uniforms);
      this._DepthPass1Material = new THREE.ShaderMaterial({
        uniforms: this._DepthPass1Uniforms,
        vertexShader: HIMAJIN.ShaderPool.DepthPass1.VertexShader,
        fragmentShader: HIMAJIN.ShaderPool.DepthPass1.FragmentShader,});
      this._DepthPass1Mesh = new THREE.Mesh(new THREE.PlaneGeometry(width, height), this._DepthPass1Material);
      this._DepthPass1 = new THREE.Scene();
      this._DepthPass1.add(this._DepthPass1Mesh);

      this._DepthPass2Uniforms = THREE.UniformsUtils.clone(HIMAJIN.ShaderPool.DepthPass2.Uniforms);
      this._DepthPass2Material = new THREE.ShaderMaterial({
        uniforms: this._DepthPass2Uniforms,
        vertexShader: HIMAJIN.ShaderPool.DepthPass2.VertexShader,
        fragmentShader: HIMAJIN.ShaderPool.DepthPass2.FragmentShader,
        defines: HIMAJIN.ShaderPool.DepthPass2.Defines, });
      this._DepthPass2 = new THREE.Scene();
      this._DepthPass2.add(new THREE.Mesh(new THREE.PlaneGeometry(width, height), this._DepthPass2Material));
      
      this._DepthPass3Uniforms = THREE.UniformsUtils.clone(HIMAJIN.ShaderPool.DepthPass3.Uniforms);
      this._DepthPass3Material = new THREE.ShaderMaterial({
        uniforms: this._DepthPass3Uniforms,
        vertexShader: HIMAJIN.ShaderPool.DepthPass3.VertexShader,
        fragmentShader: HIMAJIN.ShaderPool.DepthPass3.FragmentShader,
        defines: HIMAJIN.ShaderPool.DepthPass3.Defines, });
      this._DepthPass3 = new THREE.Scene();
      this._DepthPass3.add(new THREE.Mesh(new THREE.PlaneGeometry(width, height), this._DepthPass3Material));

      this._ColorPass1Uniforms = THREE.UniformsUtils.clone(HIMAJIN.ShaderPool.ColorPass1.Uniforms);
      this._ColorPass1Material = new THREE.ShaderMaterial({
        uniforms: this._ColorPass1Uniforms,
        vertexShader: HIMAJIN.ShaderPool.ColorPass1.VertexShader,
        fragmentShader: HIMAJIN.ShaderPool.ColorPass1.FragmentShader,
        defines: HIMAJIN.ShaderPool.ColorPass1.Defines, });
      this._ColorPass1 = new THREE.Scene();
      this._ColorPass1.add(new THREE.Mesh(new THREE.PlaneGeometry(width, height), this._ColorPass1Material));
      
      this._ColorPass2Uniforms = THREE.UniformsUtils.clone(HIMAJIN.ShaderPool.ColorPass2.Uniforms);
      this._ColorPass2Material = new THREE.ShaderMaterial({
        uniforms: this._ColorPass2Uniforms,
        vertexShader: HIMAJIN.ShaderPool.ColorPass2.VertexShader,
        fragmentShader: HIMAJIN.ShaderPool.ColorPass2.FragmentShader, });
      this._ColorPass2 = new THREE.Scene();
      this._ColorPass2.add(new THREE.Mesh(new THREE.PlaneGeometry(width, height), this._ColorPass2Material));
      
      this._CompositionCamera = new THREE.OrthographicCamera(width / -2, width / 2, height / 2, height / -2, -10000, 10000);

      this._ResetRenderTargetUniforms();
    },
    _UpdateCamera: function (frameInfo) {
      this._CameraRotation += (this._CameraRotationTarget - this._CameraRotation) * 0.05;
      this._CameraLean += (this._CameraLeanTarget - this._CameraLean) * 0.025;
      
      var r = this._CameraRotation * -Math.PI + frameInfo.TotalTime * 0.75;
      var d = this._CameraLean * 300.0 + 750.0;
      this._Camera.position.z = Math.cos(r) * d;
      this._Camera.position.x = Math.sin(r) * d;
      this._Camera.position.y = 300.0 + this._CameraLean * 500.0;
      this._Camera.lookAt(this._Scene1.position);
    },
    _UpdateScene: function (frameInfo) {
      for(var i = 0; i < _NumberOfObjects; ++i) {
        var quaternion = new THREE.Quaternion();
        this._ObjectRotationAngles[i] += frameInfo.ElapsedTime * 1.5;
        quaternion.setFromAxisAngle(this._ObjectRotationAxes[i], this._ObjectRotationAngles[i]);
        this._ObjectMeshes[i].quaternion.copy(quaternion);
      }
    },
    _UpdateLight: function (frameInfo) {
    },
    _RenderScene: function (frameInfo) {

      if(this._EnableDoF) {
        this._Scene1.overrideMaterial = null;
        this._Scene2.overrideMaterial = null;
        this._Renderer.render(this._Scene1, this._Camera, this._ColorRenderTarget, true);
        this._Renderer.render(this._Scene2, this._Camera, this._ColorRenderTarget, false);
        this._Scene1.overrideMaterial = this._DepthMaterial;
        this._Renderer.render(this._Scene1, this._Camera, this._DepthRenderTarget, true);
        this._Scene1.overrideMaterial = this._DepthPass1Material;
        this._Renderer.render(this._DepthPass1, this._CompositionCamera, this._DepthPass1Output, false);
        this._Renderer.render(this._DepthPass2, this._CompositionCamera, this._DepthPass2Output, false);
        this._Renderer.render(this._DepthPass3, this._CompositionCamera, this._DepthPass3Output, false);
        this._Renderer.render(this._ColorPass1, this._CompositionCamera, this._ColorPass1Output, false);
        this._Renderer.render(this._ColorPass2, this._CompositionCamera);
      } else {
        this._Scene1.overrideMaterial = null;
        this._Scene2.overrideMaterial = null;
...
$(document).ready(function() {
  HIMAJIN.InstancePool.demo =
    new HIMAJIN.Demo("#RenderCanvas");
});
